...
const DELETE_SIZE = structure.files.reduce((p: number, c: number) => p + c, 0) - TOTAL + UNUSED;
...
const getAtMost = (struct: any) => {
  ...
  if (totalSize >= DELETE_SIZE) atMost.push(totalSize);
...
console.log('What is the total size of that directory?', Math.min(...atMost));

// answer 1815525
// example answer 24933642
